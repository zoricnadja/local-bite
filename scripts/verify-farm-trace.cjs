// End-to-end regression: uses disposable accounts/farm and cleans only its own fixtures.
...
async function projectedTrace(qr) {
  for(let i=0;i<40;i++) {
    ...
    if(result.status===200 && result.json.data.farm_name && result.json.data.batch?.raw_materials.length) return result.json.data;
...
async function main() {
  try {
    ...
    const registered = await request('/auth/register','POST',{email:ownerEmail,password,role:'FARM_OWNER',first_name:'Test',last_name:'Owner',address:'Test address'});
    ...
    assert.equal(claims(oldToken).farm_id,null);
    const created=data(await request('/auth/farms','POST',{name:'Farma Đorđević',address:'Šumadija'},oldToken),201);
    farm=created.farm.id;
    assert.equal(claims(created.token).farm_id,farm);
    const refreshed = await request('/auth/me','GET',undefined,oldToken);
    assert.equal(refreshed.status,200);
    assert.equal(refreshed.json.farm_id,farm);
    const token=refreshed.headers.get('x-session-token');
    assert.equal(claims(token).farm_id,farm);
    for (const path of ['/orders/analytics','/productions/batches','/products/farm','/raw-materials/low-stock']) data(await request(path,'GET',undefined,token));
    const added=data(await request(`/auth/farms/${farm}/workers`,'POST',{email:workerEmail,password,first_name:'Test',last_name:'Worker',address:'Test address'},oldToken));
    worker=added.id;
    const login=await request('/auth/login','POST',{email:workerEmail,password});
    assert.equal(login.status,200);
    const workerToken=login.json.token;
    assert.equal(data(await request(`/auth/farms/${farm}`,'GET',undefined,workerToken)).name,'Farma Đorđević');
    assert.equal((await request(`/auth/users/${worker}`,'DELETE',undefined,workerToken)).status,403);
    assert.equal((await request(`/auth/farms/${farm}`,'PUT',{name:'Forbidden'},workerToken)).status,403);
    ...
    let product;
    for(let i=0;i<60;i++) {
      const stock=data(await request('/products/farm?is_active=false','GET',undefined,token));
    ...
    assert.ok(!data(await request('/products?is_active=false&farm_id='+farm,'GET',undefined,customerToken)).data.some(p=>p.id===product.id),'Customer saw Storage');
    product=data(await request('/products/'+product.id,'PUT',{price:8,is_active:true},token));
    const active=data(await request('/products?farm_id='+farm,'GET',undefined,customerToken));
    ...
    const filtered=data(await request('/orders/user/'+customerId+'?farm_id='+farm,'GET',undefined,customerToken));
    assert.equal(filtered.total,1);assert.equal(filtered.data[0].id,order.id);
    assert.equal(data(await request('/orders/user/'+customerId+'?farm_id='+crypto.randomUUID(),'GET',undefined,customerToken)).total,0);
    ...
    assert.equal(data(await request('/products/farm?is_active=true','GET',undefined,token)).total,1);
    ...
    const unsigned=enc({alg:'HS256',typ:'JWT'})+'.'+enc({sub:recoverKey,role:'ORDER_STOCK',email:'',farm_id:null,iat:now,exp:now+60});
    ...
    assert.equal(trace.farm_name,'Farma Đorđević');
    ...
    assert.equal(trace.product.farm_id,undefined); assert.equal(trace.batch.raw_materials[0].supplier,undefined);
    ...
    assert.equal((await request(`/auth/farms/${farm}/trace`,'GET',undefined,workerToken)).status,403);
    ...
    if(farm) {
      sql('orders-db','orders_db',`DELETE FROM orders WHERE farm_id='${farm}'; DELETE FROM checkout_jobs WHERE payload->'lines' @> '[{"farm_id":"${farm}"}]'::jsonb;`);
      sql('products-db','products_db',`WITH removed AS (DELETE FROM stock_reservation_items WHERE farm_id='${farm}' RETURNING reservation_id) DELETE FROM stock_reservations WHERE id IN (SELECT reservation_id FROM removed); DELETE FROM production_outputs WHERE product_id IN (SELECT id FROM products WHERE farm_id='${farm}'); DELETE FROM products WHERE farm_id='${farm}'; DELETE FROM production_states WHERE farm_id='${farm}';`);
      sql('productions-db','productions_db',`DELETE FROM production_batches WHERE farm_id='${farm}';`);
      sql('raw-materials-db','raw_materials_db',`DELETE FROM production_consumption WHERE farm_id='${farm}'; DELETE FROM raw_materials WHERE farm_id='${farm}';`);
      sql('auth-db','auth_db',`UPDATE users SET farm_id=NULL WHERE farm_id='${farm}'; DELETE FROM farms WHERE id='${farm}';`);
    }
    sql('auth-db','auth_db',`DELETE FROM users WHERE email IN ('${ownerEmail}','${workerEmail}','${customerEmail}','${otherEmail}');`);
    console.log('Removed disposable accounts and farm data.');
  }
}
