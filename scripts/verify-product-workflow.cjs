// Integration regression using an isolated business and only its own fixtures.
...
(async()=>{try{
  const registered=await api('/auth/register','POST',{email:`workflow-${run}@example.invalid`,password:randomUUID(),role:'BUSINESS_OWNER',first_name:'Workflow',last_name:'Test',address:'Test'});
  assert.equal(registered.status,201);
  user=JSON.parse(Buffer.from(registered.json.token.split('.')[1],'base64url')).sub;
  const created=data(await api('/auth/businesses','POST',{name:'Workflow regression',address:'Test'},registered.json.token),201);
  business=created.business.id;const token=created.token;
  ...
  const list=async(status)=>data(await api('/products/business?status='+status,'GET',undefined,token));
  ...
  if(business){
    sql('products',`DELETE FROM production_outputs WHERE batch_id IN(SELECT batch_id FROM products WHERE business_id='${business}'); DELETE FROM products WHERE business_id='${business}'; DELETE FROM production_states WHERE business_id='${business}';`);
    sql('productions',`DELETE FROM production_batches WHERE business_id='${business}';`);
    sql('auth',`UPDATE users SET business_id=NULL WHERE id='${user}'; DELETE FROM businesses WHERE id='${business}';`);
  }
