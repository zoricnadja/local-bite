...
const business=crypto.randomUUID(), id=crypto.randomUUID(), legacy=crypto.randomUUID();
...
(async()=>{try{
 const event={schema_version:1,source:'productions',sequence:2,entity_type:'production_batches',entity_id:id,operation:'DELETE',data:{id,business_id:business,status:'COMPLETED',output_name:'Test',output_type:'cheese',output_unit:'kg',output_quantity:2}};
 ...
 assert.equal(sql(`SELECT count(*) FROM products WHERE business_id='${business}'`),'0','Stale or legacy event created inventory');
...
}finally{sql(`DELETE FROM production_states WHERE business_id='${business}';`);}})().catch(e=>{console.error(e.message);process.exitCode=1;});
