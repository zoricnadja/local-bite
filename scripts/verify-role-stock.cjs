// Local integration check. Uses a unique test farm and removes only its fixtures.
...
function token(role) {
  ...
  const data = enc({ alg:'HS256',typ:'JWT' })+'.'+enc({ sub:crypto.randomUUID(),email:'integration@example.invalid',role,farm_id:farm,iat:now,exp:now+600 });
  return data+'.'+crypto.createHmac('sha256',settings.JWT_SECRET).update(data).digest('base64url');
}
const owner=token('FARM_OWNER'), worker=token('WORKER'), customer=token('CUSTOMER');
...
function cleanup() {
  // farm is generated above, never accepted from external input.
  const user=settings.POSTGRES_USER || 'postgres';
  for(const [container,db,sql] of [
    ['productions-db','productions_db',`DELETE FROM production_batches WHERE farm_id='${farm}';`],
    ['raw-materials-db','raw_materials_db',`DELETE FROM production_consumption WHERE farm_id='${farm}'; DELETE FROM raw_materials WHERE farm_id='${farm}';`],
