...
erDiagram
    USER ||--o{ FARM : owns
    FARM ||--o{ USER : assigned
    FARM ||--o{ RAW_MATERIAL : manages
    FARM ||--o{ PRODUCTION_BATCH : produces
...
| `farm_id` | Opcioni UUID; nije deklarisan FK prema farms |
...
| Polja | Tip / značenje |
|---|---|
| `id`, `farm_id` | UUID PK i obavezna tenant referenca |
...
Indeksi: farma, tip i `(farm_id, is_deleted)`. Niska zaliha je količina manja ili jednaka postavljenom pragu. Ne postoji SQL CHECK nenegativnosti količine u početnoj tabeli; neke putanje ga obezbeđuju aplikacionim uslovom, a neke imaju slabiju validaciju.

### production_consumption

`operation_id UUID PK`; `farm_id UUID`; `raw_material_id UUID REFERENCES raw_materials(id)`; `quantity NUMERIC(12,3) CHECK(quantity > 0)`; `unit VARCHAR(50)`; `released BOOLEAN DEFAULT false`. Sva polja osim default vrednosti su obavezna. Operation ID je identitet poslovnog pokušaja utroška; nije ID HTTP zahteva ili korisnika.
...
`operation_id UUID PK`, `farm_id UUID NOT NULL`, `created_at TIMESTAMPTZ`. Release upisuje tombstone pre vraćanja količine. Kasni consume istog ID-a se odbija i ne može ponovo umanjiti zalihu.

## 5. Productions baza

### production_batches

Osnovna polja: `id UUID PK`, `farm_id UUID`, `name VARCHAR(255)`, `process_type VARCHAR(100)`, opcioni `start_date DATE`, `end_date DATE`, `notes TEXT`, `status VARCHAR(20)` default PLANNED sa CHECK skupom statusa, `is_deleted BOOLEAN`, `created_at/updated_at TIMESTAMP`.
...
`id UUID PK`; `batch_id UUID REFERENCES production_batches(id) ON DELETE CASCADE`; `farm_id UUID`; `step_order INT`; `name VARCHAR(255)`; opcioni `description TEXT`, `duration_hours NUMERIC(8,2)`, `temperature NUMERIC(6,2)`; timestamp-i. UNIQUE `(batch_id, step_order)` i indeks batch-a. Temperatura je opisni podatak procesa, ne očitavanje senzora.

### batch_raw_materials

`id UUID PK`; `batch_id` lokalni FK sa cascade; `farm_id`; `raw_material_id` udaljena referenca; obavezni `raw_material_name VARCHAR(255)`, `material_type VARCHAR(100)`, `quantity_used NUMERIC(12,3)`, `unit VARCHAR(50)`; opcioni `origin`, `supplier`, `harvest_date`, `received_date`, `expiry_date`.
...
Obe tabele koriste `operation_id UUID PK`, `farm_id`, `created_at` i opcioni `completed_at`. Intent se nezavisno commit-uje pre udaljenog consume poziva. Release job nastaje pri uklanjanju veze, otkazivanju ili brisanju serije. Recovery obrađuje nezavršene redove idempotentno.
...
Indeksi: farma, tip, `(farm_id, is_active, is_deleted)`, QR. `is_active` u API odgovoru može biti efektivna vrednost izračunata zajedno sa `production_states`, a ne samo fizička kolona.
...
`batch_id UUID PRIMARY KEY`, `farm_id UUID`, `status TEXT`, `sequence BIGINT`, `deleted BOOLEAN DEFAULT false`. Ovo je lokalna kopija statusa serije ažurirana događajima. Output consumer proverava sekvencu pod istim transakcionim advisory lock-om pre promene statusa ili kreiranja izlaza.

### stock_reservations i stock_reservation_items

`stock_reservations` ima `id UUID PK`, kanonski JSON payload i vreme nastanka. Stavke imaju složeni ključ `(reservation_id, product_id)`, FK prema rezervaciji/proizvodu, `farm_id`, pozitivnu `NUMERIC(12,3)` količinu i `released`. Jedan transaction zaključava operaciju i uslovno umanjuje sve proizvode; konflikt vraća ceo transaction.

## 7. Orders baza

### orders

`id UUID PK`, `farm_id UUID NOT NULL`, opcioni `customer_id UUID`, `customer_name/customer_email VARCHAR(255)`, `status VARCHAR(20)` default PENDING sa CHECK skupom, `total_price NUMERIC(12,2)` default 0, `notes TEXT`, `is_deleted BOOLEAN`, `created_at/updated_at TIMESTAMP`.

SQL dozvoljava null customer_id radi anonimne prodaje u početnom dizajnu, ali aktivan HTTP create prima samo CUSTOMER i servis upisuje ID. To nije implementirana walk-in prodajna funkcija. Indeksi pokrivaju farmu, kupca, status po farmi i datum po farmi.
...
Veza mapira nastalu porudžbinu na checkout rezervaciju i farmu. Release job ima `order_id PK`, checkout/farm i `completed_at`; upisuje se u istoj transakciji kao otkazivanje ili dozvoljeno brisanje porudžbine.
...
Složeni PK `(source, entity_type, entity_id)`; `sequence BIGINT`, `deleted BOOLEAN`, `data JSONB`, `projected_at TIMESTAMPTZ`. Indeksi po JSON farm_id, customer_id, batch_id i qr_token ubrzavaju najvažnije pretrage. Deleted red se zadržava radi odbacivanja zakasnelog starog stanja.
...
Sirovine, serije, proizvodi i porudžbine uglavnom koriste `is_deleted`. Koraci i veze sirovina brišu se fizički kroz odgovarajuće repozitorijume; korisnici i farme takođe imaju fizičko brisanje. Soft-delete roditelja nije SQL DELETE i sam po sebi ne pokreće `ON DELETE CASCADE` nad njegovom decom.
...
Veza mapira svaku nastalu porudžbinu na checkout rezervaciju i farmu. Release job ima `order_id PK`, checkout/farm i `completed_at`; upisuje se u istoj transakciji kao otkazivanje ili dozvoljeno brisanje porudžbine.
