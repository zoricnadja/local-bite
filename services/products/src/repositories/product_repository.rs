...
impl ProductRepository {
    ...
    async fn list_filtered(&self, farm:Option<Uuid>, q:&ListQuery)->AppResult<(Vec<Product>,i64)> {
        ...
        let filter=format!("FROM products p WHERE NOT p.is_deleted AND ($1::uuid IS NULL OR p.farm_id=$1) AND ($2='' OR p.product_type ILIKE $2) AND ($3='' OR p.name ILIKE '%'||$3||'%') AND ($4::bool IS NULL OR {}=$4)",available);
        let select=format!("SELECT p.id,p.farm_id,p.name,p.product_type,p.description,p.quantity,p.unit,p.price,p.expiry_date,p.batch_id,p.image_path,p.qr_token,p.qr_path,{} AS is_active,p.is_deleted,p.created_at,p.updated_at {} ORDER BY p.created_at DESC,p.id LIMIT $5 OFFSET $6",available,filter);
        let items=sqlx::query_as::<_,Product>(&select).bind(farm).bind(q.product_type.as_deref().unwrap_or("")).bind(q.search.as_deref().unwrap_or("")).bind(active).bind(q.limit()).bind(q.offset()).fetch_all(&self.pool).await?;
        let total=sqlx::query_scalar::<_,i64>(&format!("SELECT count(*) {}",filter)).bind(farm).bind(q.product_type.as_deref().unwrap_or("")).bind(q.search.as_deref().unwrap_or("")).bind(active).fetch_one(&self.pool).await?;
        Ok((items,total))
    }
    ...
    pub async fn find_by_id_and_farm(&self, id: Uuid, farm_id: Uuid) -> AppResult<Product> {
        sqlx::query_as!(
            ...
            SELECT id, farm_id, name, product_type, description, quantity, unit, price,
                   expiry_date, batch_id, image_path, qr_token, qr_path, is_active, is_deleted,
    ...
    pub async fn update(&self, id: Uuid, farm_id: Uuid, p: UpdateParams) -> AppResult<Product> {
        Ok(sqlx::query_as::<_, Product>(r#"
            UPDATE products SET name=$1, product_type=$2, description=$3,
                ...
                unit=$5, price=$6, batch_id=$7, is_active=$8, expiry_date=$11
        ...
        .bind(p.expiry_date).fetch_one(&self.pool).await?)
    }
