...
#[derive(Serialize)]
pub struct PublicProvenance { pub product:PublicProduct, pub business_name:Option<String>, pub batch:Option<PublicBatch> }

impl From<ProvenanceResponse> for PublicProvenance {
    fn from(p:ProvenanceResponse)->Self {
        Self {product:PublicProduct{name:p.product.name,product_type:p.product.product_type,description:p.product.description,expiry_date:p.product.expiry_date,qr_token:p.product.qr_token},business_name:p.business_name,
