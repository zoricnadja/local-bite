...
#[derive(Serialize)]
pub struct PublicStep { pub step_order:i32, pub name:String, pub description:Option<String>, pub duration_hours:Option<f64>, pub temperature:Option<f64> }
#[derive(Serialize)]
pub struct PublicBatch { pub name:String, pub process_type:String, pub start_date:Option<String>, pub end_date:Option<String>, pub status:String, pub steps:Vec<PublicStep>, pub raw_materials:Vec<PublicMaterial> }
#[derive(Serialize)]
...
impl From<ProvenanceResponse> for PublicProvenance {
    fn from(p:ProvenanceResponse)->Self {
        Self {product:PublicProduct{name:p.product.name,product_type:p.product.product_type,description:p.product.description,expiry_date:p.product.expiry_date,qr_token:p.product.qr_token},farm_name:p.farm_name,
            batch:p.batch.map(|b|PublicBatch{name:b.name,process_type:b.process_type,start_date:b.start_date,end_date:b.end_date,status:b.status,
                steps:b.steps.into_iter().map(|s|PublicStep{step_order:s.step_order,name:s.name,description:s.description,duration_hours:s.duration_hours,temperature:s.temperature}).collect(),
