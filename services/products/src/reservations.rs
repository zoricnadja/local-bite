...
pub async fn reserve(AuthClaims(claims): AuthClaims, Extension(pool): Extension<PgPool>,
    ...
    for item in items {
        ...
        sqlx::query("INSERT INTO stock_reservation_items(reservation_id,product_id,farm_id,quantity) VALUES($1,$2,$3,$4)")
            .bind(id).bind(item.product_id).bind(row.get::<Uuid,_>("farm_id")).bind(quantity).execute(&mut *tx).await?;
    }
    tx.commit().await?; Ok(no_content())
}

pub async fn release(AuthClaims(claims):AuthClaims,Extension(pool):Extension<PgPool>,Path((id,farm)):Path<(Uuid,Uuid)>)->AppResult<Response>{
    service_auth::require(&claims,"ORDER_STOCK",id)?;
    if claims.farm_id!=Some(farm){return Err(AppError::Forbidden("Reservation farm mismatch".into()));}
    let mut tx=pool.begin().await?;
    sqlx::query("SELECT pg_advisory_xact_lock(hashtextextended($1,11))").bind(id.to_string()).execute(&mut *tx).await?;
    let rows=sqlx::query("SELECT product_id,quantity FROM stock_reservation_items WHERE reservation_id=$1 AND farm_id=$2 AND NOT released ORDER BY product_id FOR UPDATE")
        .bind(id).bind(farm).fetch_all(&mut *tx).await?;
    for row in rows {
