...
impl ProductService {
    ...
    pub async fn find_all_by_business_id(
        ...
        let (items, total) = self
            .product_repository
            .find_all_by_business_id(business_id, q)
            .await?;
    ...
    pub async fn create(&self, business_id: Uuid, req: CreateProductRequest) -> AppResult<Product> {
        let _ = (business_id,req);
    ...
    pub async fn update(
        ...
        if req.is_active.unwrap_or(existing.is_active) {
            self.validate_batch(business_id, req.batch_id.or(existing.batch_id), req.expiry_date.or(existing.expiry_date)).await?;
    ...
    pub async fn delete(&self, id: Uuid, business_id: Uuid) -> AppResult<()> {
        let product = self.product_repository.find_by_id_and_business(id, business_id).await?;
        if product.status == "PRODUCTION" {
            return Err(AppError::BadRequest("Remove planned products through their production batch".into()));
        }
        let rows = self.product_repository.soft_delete(id, business_id).await?;
        if rows == 0 {
    ...
    async fn validate_batch(&self, business_id: Uuid, batch_id: Option<Uuid>, expiry: Option<chrono::NaiveDate>) -> AppResult<()> {
        ...
        if let Some(id) = batch_id {
            let token = super::provenance_service::trace_token(business_id, Some(id))?;
            let batch = crate::dtos::clients::fetch_batch(id, &token).await
                .map_err(|_| AppError::BadRequest("Select an available production batch from your business".into()))?;
...
pub fn authorize_read(product:&Product,claims:&common::jwt::Claims)->AppResult<()> {
    let allowed=match claims.role.as_str(){"SYSTEM_ADMIN"=>true,"CUSTOMER"=>product.is_active && product.expiry_date.is_none_or(|d|d>=Utc::now().date_naive()),"BUSINESS_OWNER"|"WORKER"=>claims.business_id==Some(product.business_id),_=>false};
