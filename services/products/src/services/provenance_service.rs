...
impl ProvenanceService {
    ...
    async fn build(&self, product: Product, _token: &str) -> AppResult<ProvenanceResponse> {
        let token = trace_token(product.farm_id, Some(product.id))?;
        ...
        // All farm/production/material reads come only from the separate query database.
        projected.product = product;
        Ok(projected)
    }
}

// Never sent to the browser: read-only, one farm and one batch, expires in a minute.
pub fn trace_token(farm_id: Uuid, batch_id: Option<Uuid>) -> AppResult<String> {
    ...
    let claims = common::jwt::Claims {
        ...
        role: "TRACEABILITY".into(), farm_id: Some(farm_id), iat: now, exp: now + 60,
    };
