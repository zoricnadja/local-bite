...
use common::{
    ...
    middleware::{require_farm, AuthClaims},
    response::no_content,
};
...
pub async fn consume(
    ...
    for item in items {
        ...
        let inserted = sqlx::query("INSERT INTO production_consumption (operation_id, farm_id, raw_material_id, quantity, unit) VALUES ($1,$2,$3,$4,$5) ON CONFLICT DO NOTHING")
            .bind(item.operation_id).bind(farm_id).bind(item.raw_material_id).bind(&quantity).bind(item.unit.trim())
            .execute(&mut *tx).await?.rows_affected();
        if inserted == 0 {
            let row = sqlx::query("SELECT farm_id, raw_material_id, quantity, unit, released FROM production_consumption WHERE operation_id=$1 FOR UPDATE")
                .bind(item.operation_id).fetch_one(&mut *tx).await?;
            if row.get::<Uuid, _>("farm_id") != farm_id
        ...
        let rows = sqlx::query("UPDATE raw_materials SET quantity=quantity-$1 WHERE id=$2 AND farm_id=$3 AND NOT is_deleted AND unit=$4 AND quantity >= $1")
            .bind(&quantity).bind(item.raw_material_id).bind(farm_id).bind(item.unit.trim())
            .execute(&mut *tx).await?.rows_affected();
        if rows == 0 {
            return Err(AppError::BadRequest(
                "Not enough stock, mismatched unit, or material unavailable on this farm".into(),
            ));
        }
    }
    tx.commit().await?;
...
pub async fn release(
    ...
    for id in ids {
        ...
        sqlx::query("INSERT INTO consumption_tombstones(operation_id,farm_id) VALUES($1,$2) ON CONFLICT DO NOTHING").bind(id).bind(farm_id).execute(&mut *tx).await?;
        if let Some(row) = sqlx::query("UPDATE production_consumption SET released=TRUE WHERE operation_id=$1 AND farm_id=$2 AND NOT released RETURNING raw_material_id, quantity")
            .bind(id).bind(farm_id).fetch_optional(&mut *tx).await? {
            sqlx::query("UPDATE raw_materials SET quantity=quantity+$1 WHERE id=$2 AND farm_id=$3")
                .bind(row.get::<BigDecimal,_>("quantity")).bind(row.get::<Uuid,_>("raw_material_id")).bind(farm_id)
...
mod tests {
    ...
    fn claims(farm_id: Uuid, role: &str) -> AuthClaims {
        AuthClaims(Claims {
            sub: Uuid::nil(),
    ...
    async fn consumption_is_atomic_scoped_and_idempotent() {
        let pool = sqlx::postgres::PgPoolOptions::new()
            ...
            .await
            .unwrap();
        sqlx::raw_sql("CREATE TEMP TABLE raw_materials (id UUID PRIMARY KEY, farm_id UUID NOT NULL, quantity NUMERIC(12,3), unit TEXT, is_deleted BOOLEAN DEFAULT FALSE)").execute(&pool).await.unwrap();
        ...
        sqlx::raw_sql("CREATE TEMP TABLE consumption_tombstones (operation_id UUID PRIMARY KEY, farm_id UUID NOT NULL, created_at TIMESTAMPTZ DEFAULT now())").execute(&pool).await.unwrap();
        ...
        for id in [material, second] {
            sqlx::query(
                "INSERT INTO raw_materials(id,farm_id,quantity,unit) VALUES ($1,$2,10,'kg')",
            )
            .bind(id)
            .bind(farm)
            .execute(&pool)
