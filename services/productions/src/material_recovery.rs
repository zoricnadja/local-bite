...
use sqlx::PgPool;
use uuid::Uuid;
pub async fn journal(pool:&PgPool,tx:&mut sqlx::Transaction<'_,sqlx::Postgres>,id:Uuid,business:Uuid)->common::errors::AppResult<()> {
    ...
    sqlx::query("INSERT INTO material_intents(operation_id,business_id) VALUES($1,$2) ON CONFLICT DO NOTHING").bind(id).bind(business).execute(pool).await?;
    Ok(())
}
async fn release(business:Uuid,id:Uuid)->anyhow::Result<()> {
    ...
    reqwest::Client::new().post(format!("{base}/internal/production-consumption/release"))
        .bearer_auth(common::service_auth::token("MATERIAL_STOCK",Uuid::nil(),Some(business))?).json(&vec![id])
        .timeout(std::time::Duration::from_secs(10)).send().await?.error_for_status()?;
    Ok(())
}
pub async fn recover(pool:&PgPool)->anyhow::Result<()> {
    let pending:Vec<(Uuid,Uuid)>=sqlx::query_as("SELECT operation_id,business_id FROM material_intents WHERE completed_at IS NULL AND created_at<now()-interval '30 seconds' ORDER BY created_at LIMIT 50").fetch_all(pool).await?;
    for (id,business) in pending {
        ...
        if !exists {release(business,id).await?;}
        sqlx::query("UPDATE material_intents SET completed_at=now() WHERE operation_id=$1").bind(id).execute(&mut *tx).await?;
        tx.commit().await?;
    }
    let jobs:Vec<(Uuid,Uuid)>=sqlx::query_as("SELECT operation_id,business_id FROM material_release_jobs WHERE completed_at IS NULL ORDER BY created_at LIMIT 50").fetch_all(pool).await?;
    for (id,business) in jobs {
        release(business,id).await?;
