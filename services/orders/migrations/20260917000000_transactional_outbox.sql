...
CREATE TRIGGER integration_event AFTER INSERT OR UPDATE OR DELETE ON orders FOR EACH ROW EXECUTE FUNCTION capture_integration_event();
INSERT INTO integration_outbox(entity_type,entity_id,operation,data) SELECT 'orders',id,'INSERT',integration_payload('orders',to_jsonb(t)) FROM orders t;

CREATE TRIGGER integration_event AFTER INSERT OR UPDATE OR DELETE ON order_items FOR EACH ROW EXECUTE FUNCTION capture_integration_event();
INSERT INTO integration_outbox(entity_type,entity_id,operation,data) SELECT 'order_items',id,'INSERT',integration_payload('order_items',to_jsonb(t)) FROM order_items t;
