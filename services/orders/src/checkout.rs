...
struct Line { product_id:Uuid, business_id:Uuid, name:String, kind:String, unit:String, quantity:String, unit_price:String }
...
pub async fn submit(service:&OrderService,id:Uuid,customer:Uuid,email:&str,req:CreateOrderRequest,token:&str)->AppResult<Value>{
    ...
    for (product_id,quantity) in quantities {
        ...
        lines.push(Line{product_id,business_id:p.business_id.ok_or_else(||AppError::BadRequest("Product has no producer".into()))?,name:p.name,kind:p.product_type,unit:p.unit,quantity:quantity.to_string(),unit_price:p.price.to_string()});
    }
...
async fn process(service:&OrderService,id:Uuid)->AppResult<Value>{
    ...
    let mut businesses:BTreeMap<Uuid,Vec<NewOrderItem>>=BTreeMap::new();
    for l in lines {
        businesses.entry(l.business_id).or_default().push(NewOrderItem{product_id:l.product_id,product_name:l.name,product_type:l.kind,business_id:l.business_id,unit:l.unit,
    ...
    for (business,items) in businesses {
        let total=items.iter().map(|l|(&l.quantity*&l.unit_price).round(2)).fold(BigDecimal::from(0),|a,b|a+b);
        let order=service.order_repository.insert(&mut tx,business,row.get("customer_id"),payload["customer_name"].as_str(),payload["customer_email"].as_str().unwrap_or_default().into(),payload["notes"].as_str(),&total).await?;
        let saved=service.order_item_repository.insert_batch(&mut tx,order.id,&items.iter().collect::<Vec<_>>()).await?;
        sqlx::query("INSERT INTO order_stock_links(order_id,checkout_id,business_id) VALUES($1,$2,$3)").bind(order.id).bind(id).bind(business).execute(&mut *tx).await?;
...
pub async fn release_pending(service:&OrderService)->AppResult<()> {
    let jobs:Vec<(Uuid,Uuid,Uuid)>=sqlx::query_as("SELECT order_id,checkout_id,business_id FROM stock_release_jobs WHERE completed_at IS NULL ORDER BY created_at LIMIT 50").fetch_all(&service.order_repository.pool).await?;
    let base=std::env::var("PRODUCTS_SERVICE_URL").unwrap_or_else(|_|"http://products-service:3003".into());
    for (order,id,business) in jobs {
        let result=reqwest::Client::new().post(format!("{base}/internal/reservations/{id}/release/{business}"))
            .bearer_auth(service_auth::token("ORDER_STOCK",id,Some(business))?).timeout(std::time::Duration::from_secs(10)).send().await;
