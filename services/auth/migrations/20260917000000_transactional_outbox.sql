...
CREATE TRIGGER integration_event AFTER INSERT OR UPDATE OR DELETE ON farms FOR EACH ROW EXECUTE FUNCTION capture_integration_event();
INSERT INTO integration_outbox(entity_type,entity_id,operation,data) SELECT 'farms',id,'INSERT',integration_payload('farms',to_jsonb(t)) FROM farms t;

CREATE TRIGGER integration_event AFTER INSERT OR UPDATE OR DELETE ON users FOR EACH ROW EXECUTE FUNCTION capture_integration_event();
INSERT INTO integration_outbox(entity_type,entity_id,operation,data) SELECT 'users',id,'INSERT',integration_payload('users',to_jsonb(t)) FROM users t;
