...
use common::{errors::{AppError, AppResult}, middleware::{AuthClaims, require_farm, require_role}, response::ok};
...
async fn farm_entities(pool: &PgPool, kind: &str, farm: Option<Uuid>) -> AppResult<Vec<Value>> {
    Ok(sqlx::query_scalar("SELECT data FROM projection_entities WHERE entity_type=$1 AND NOT deleted AND ($2::text IS NULL OR data->>'farm_id'=$2) ORDER BY data->>'created_at' DESC,entity_id")
        .bind(kind).bind(farm.map(|id| id.to_string())).fetch_all(pool).await?)
}

pub async fn dashboard(AuthClaims(claims): AuthClaims, Extension(pool): Extension<PgPool>) -> AppResult<Response> {
    require_role(&claims, &["CUSTOMER", "WORKER", "FARM_OWNER", "SYSTEM_ADMIN"])?;
    ...
    let farm = if claims.role == "SYSTEM_ADMIN" { None } else { Some(require_farm(&claims)?) };
    let (materials, batches, products, orders) = tokio::try_join!(
        farm_entities(&pool,"raw_materials",farm), farm_entities(&pool,"production_batches",farm),
        farm_entities(&pool,"products",farm), farm_entities(&pool,"orders",farm))?;
...
pub async fn provenance(AuthClaims(claims): AuthClaims, Path(id): Path<Uuid>, Extension(pool): Extension<PgPool>) -> AppResult<Response> {
    ...
    let farm_id=product["farm_id"].as_str().and_then(|id| id.parse::<Uuid>().ok())
        .ok_or_else(|| AppError::Internal(anyhow::anyhow!("Invalid projected farm id")))?;
    if claims.farm_id != Some(farm_id) { return Err(AppError::Forbidden("Traceability scope mismatch".into())); }
    let farm=entity(&pool,"farms",farm_id).await?;
    ...
    if let Some(batch_id)=product["batch_id"].as_str().and_then(|id| id.parse::<Uuid>().ok()) {
        if let Some(mut b)=entity(&pool,"production_batches",batch_id).await? {
            if b["farm_id"] != product["farm_id"] { return Err(AppError::Forbidden("Production belongs to another farm".into())); }
            let materials: Vec<Value> = sqlx::query_scalar("SELECT data FROM projection_entities WHERE entity_type='batch_raw_materials' AND NOT deleted AND data->>'batch_id'=$1 AND data->>'farm_id'=$2 ORDER BY entity_id")
                .bind(batch_id.to_string()).bind(farm_id.to_string()).fetch_all(&pool).await?;
            let steps: Vec<Value> = sqlx::query_scalar("SELECT data FROM projection_entities WHERE entity_type='process_steps' AND NOT deleted AND data->>'batch_id'=$1 AND data->>'farm_id'=$2 ORDER BY (data->>'step_order')::int,entity_id")
                .bind(batch_id.to_string()).bind(farm_id.to_string()).fetch_all(&pool).await?;
    ...
    Ok(ok(json!({"product":product,"farm_name":farm.as_ref().map(|f| &f["name"]),"batch":batch})))
}
...
pub async fn producers(AuthClaims(claims):AuthClaims, Extension(pool):Extension<PgPool>)->AppResult<Response> {
    require_role(&claims,&["CUSTOMER","FARM_OWNER","WORKER","SYSTEM_ADMIN"])?;
    let rows:Vec<Value>=sqlx::query_scalar("SELECT jsonb_build_object('id',entity_id,'name',data->>'name') FROM projection_entities WHERE entity_type='farms' AND NOT deleted ORDER BY data->>'name',entity_id").fetch_all(&pool).await?;
    Ok(ok(rows))
}
